...
type MediaQueryFunctionParams = [TemplateStringsArray | CSSObject, ...SimpleInterpolation[]]

const mediaQuery = (...query: MediaQueryFunctionParams) => (...rules: MediaQueryFunctionParams) => css`
  ${!query[0][0].startsWith('@') ? '@media ' : ''}
  ${css(...query)} {
    ${css(...rules)}
  }
`
